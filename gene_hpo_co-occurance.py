...
for s in c:
    for i in s:
        labels[i]['Community'] = str(sub_community_count) + '   '
...
it = 0
H = G.subgraph(
    list(c[1]) + list(c[2]) + list(c[3]) + list(c[4]) + list(c[5]) + list(c[6]) + list(c[7]) + list(c[8]) + list(
        c[9]) + list(c[10]))  # + list(c[11])+ list(c[12])+ list(c[13])+ list(c[14]))
sublab = {}
the_map = {}
...
coms = ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven', 'twelve', 'thirteen',
        'fourteen']
# add the community as meta data to webweb
sub_community_count = 0
for s in c[1:]:
    ...
    for i in s:
        print(i)
        sublab[the_map[i]]['Community'] = coms[sub_community_count]
    sub_community_count += 1
...
shifted_G = shift_up_tree(g, G_hpo, 6, 6)
OG_shift = shifted_G.copy()
print(len(shifted_G.nodes))
# TODO redo the community detection stuff
